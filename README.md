...
La funzione Labs 3D accoda le sole foto esterne (24–80 scatti, almeno 3 per ciascuna delle 8 aree) a un worker CPU facoltativo. COLMAP calcola l’allineamento delle fotocamere; OpenMVS esegue la ricostruzione densa CPU, crea e texturizza una mesh e la esporta come GLB archiviato in PostgreSQL. Foto di interni e cruscotto sono escluse. Il worker usa `/tmp` solo durante il calcolo: i risultati persistono nel database. La ricostruzione può impiegare molto tempo e produrre lacune/deformazioni, specialmente su vernice lucida o superfici prive di texture.

Il worker **non viene creato dal Blueprint principale**. Per abilitarlo, in Render crea un Blueprint indicando `infra/photogrammetry/render.yaml`, inserisci in `DATABASE_URL` l’Internal Database URL dello stesso PostgreSQL usato dall’app e scegli il piano del worker. È un servizio aggiuntivo con costo ricorrente: la funzione rimane disattivata/offline finché non viene avviato. Non salvare l’External Database URL o altri segreti nella repository. Vedi [`infra/photogrammetry/README.md`](infra/photogrammetry/README.md) per requisiti, limiti e licenze.
